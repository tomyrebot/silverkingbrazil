(function ($) {
    ...
    document.querySelectorAll('.read-more-btn').forEach(button => {
        button.addEventListener('click', event => {
            const expandableText = button.closest('.post-slider__info').querySelector('.expandable-text');
            const targetElement = event.target;
            const id = targetElement.id;
            const buttonNumber = id.split('-')[2];
            const readMoreId = `#read-more-${buttonNumber}`;
            const readLessId = `#read-less-${buttonNumber}`;
